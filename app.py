...
scatter = alt.Chart(df).mark_point().encode(
    alt.X('x', title='X Axis (0-100)'),
    alt.Y('y', title='Y Axis (Rand 0-1)'),
    color=alt.value("red"),
    shape=alt.value('triangle'),
    tooltip=[
        alt.Tooltip("x", title="Some X Value"),
        alt.Tooltip("y", title="Some Y I guess"),
    ])
...
- Changed Height (made bars thinner)
- Change 2
"""
)
